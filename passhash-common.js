 ...
 *
 * ***** END LICENSE BLOCK ***** */
.import "passhash-sha1.js" as Sha1

var PassHashCommon = {
    "host"// Artificial host name used for for saving to the password database
    : "passhash.passhash",
    "log": function (msg) {
    ...
    "loadOptions": function () {
        ...
        var prefs = Components.classes["@mozilla.org/preferences-service;1"].getService(
                    Components.interfaces.nsIPrefService).getBranch("passhash.")
        var forceSave = false
        ...
        if (prefs.prefHasUserValue("optHashWordSizeDefault"))
            opts.hashWordSizeDefault = prefs.getIntPref(
                        "optHashWordSizeDefault")
    ...
    "loadSecureValue": function (option, name, suffix, valueDefault) {
        return (this.hasLoginManager(
                    ) ? this.loadLoginManagerValue(
                            option, name, suffix,
                            valueDefault) : this.loadPasswordManagerValue(
                            option, name, suffix, valueDefault))
    },
    ...
    "loadPasswordManagerValue": function (option, name, suffix, valueDefault) {
        ...
        var found = false
        if (option && suffix != null) {
            var passwordManager = Components.classes["@mozilla.org/passwordmanager;1"].getService(
                        Components.interfaces.nsIPasswordManager)
            var e = passwordManager.enumerator
            while (!found && e.hasMoreElements()) {
                try {
                    var pass = e.getNext().QueryInterface(
    ...
    "saveSecureValue": function (option, name, suffix, value) {
        return (this.hasLoginManager(
                    ) ? this.saveLoginManagerValue(
                            option, name, suffix,
                            value) : this.savePasswordManagerValue(
                            option, name, suffix, value))
    },
    "saveLoginManagerValue": function (option, name, suffix, value) {
        ...
        var newLogin = Components.classes["@mozilla.org/login-manager/loginInfo;1"].createInstance(
    ...
    : function (input) {
        ...
        if (h.length > 2 && h[h.length - 1].length == 2
                && h[h.length - 2] == "co")
    ...
    : function (siteTag, masterKey, hashWordSize, requireDigit, requirePunctuation, requireMixedCase, restrictSpecial, restrictDigits) {
        // Start with the SHA1-encrypted master key/site tag.
        var s = Sha1.b64_hmac_sha1(masterKey, siteTag)
        ...
        if (restrictDigits)
            ...
            if (requireDigit)
                s = PassHashCommon.injectSpecialCharacter(s, 0, 4, sum,
                                                          hashWordSize, 48, 10)
            if (requirePunctuation && !restrictSpecial)
                s = PassHashCommon.injectSpecialCharacter(s, 1, 4, sum,
                                                          hashWordSize, 33, 15)
            if (requireMixedCase) {
                s = PassHashCommon.injectSpecialCharacter(s, 2, 4, sum,
                                                          hashWordSize, 65, 26)
                s = PassHashCommon.injectSpecialCharacter(s, 3, 4, sum,
                                                          hashWordSize, 97, 26)
            }
    ...
    : function (sInput, offset, reserved, seed, lenOut, cStart, cNum) {
        ...
        var sInject = String.fromCharCode(((seed + sInput.charCodeAt(
                                                pos)) % cNum) + cStart)
        var sTail = (pos + 1 < sInput.length ? sInput.substring(
                                                   pos + 1, sInput.length) : "")
    ...
    : function () {
        ...
        while (iSiteTag < siteTags.length || iMasterKey < masterKeys.length
               || iOption < options.length) {
            // Find the lowest domain name from the three waiting values
            var next = null
            if (iSiteTag < siteTags.length
                    && (next == null || siteTags[iSiteTag].name < next))
                next = siteTags[iSiteTag].name
            if (iMasterKey < masterKeys.length
                    && (next == null || masterKeys[iMasterKey].name < next))
    ...
    : function (siteTags, masterKeys, options) {
        ...
        passwordManager.QueryInterface(
        ...
        while (passwordEnumerator.hasMoreElements()) {
            try {
                var pw = passwordEnumerator.getNext().QueryInterface(
                            Components.interfaces.nsIPasswordInternal)
    ...
    "getResourceFile": function (uri) {
        var handler = Components.classes["@mozilla.org/network/protocol;1?name=file"].createInstance(
                    Components.interfaces.nsIFileProtocolHandler)
        var urlSrc = Components.classes["@mozilla.org/network/standard-url;1"].createInstance(
                    Components.interfaces.nsIURL)
        urlSrc.spec = uri
        var chromeReg = Components.classes["@mozilla.org/chrome/chrome-registry;1"].getService(
    ...
    "openInputFile": function (fileIn) {
        var streamIn = Components.classes["@mozilla.org/network/file-input-stream;1"].createInstance(
                    Components.interfaces.nsIFileInputStream)
        streamIn.init(fileIn, 0x01, 0x12 /*0444*/
                      , 0)
        streamIn.QueryInterface(Components.interfaces.nsILineInputStream)
        return streamIn
    },
    "openOutputFile": function (fileOut) {
        var streamOut = Components.classes["@mozilla.org/network/file-output-stream;1"].createInstance(
                    Components.interfaces.nsIFileOutputStream)
        streamOut.init(fileOut, 0x02 | 0x08 | 0x20, 0x11e /*0664*/
                       , 0) // write, create, truncate
        return streamOut
    },
    ...
    "pickHTMLFile": function (titleTag, defaultName) {
        var title = document.getElementById(
                    "pshOpt_strings").getString(titleTag)
